...
export default function EditMateriasPage() {
  ...
  const [article, setArticle] = useState<RaioxArticle | null>(null);
  const [loading, setLoading] = useState(true);
  const id = params.id as string;

  // Bind the action with the article ID and slug
  const updateActionWithId = article ? updateRaioxArticle.bind(null, id, article.slug) : null;
  const [state, formAction] = useActionState(updateActionWithId || (async () => initialState), initialState);
  
  useEffect(() => {
    const fetchArticle = async () => {
      setLoading(true);
      const newsArticle = await getRaioxById(id);
      if (newsArticle) {
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      <Card>
        <CardHeader>
          <CardTitle>Editar raio-x</CardTitle>
          <CardDescription>
            Faça alterações na raio-x e salve para publicar.
          </CardDescription>
        </CardHeader>
